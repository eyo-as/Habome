...
const MOCK_PROPERTY: Property = {
  ...
  description:
    'Stunning luxury penthouse with panoramic city views. This exceptional property features premium finishes, floor-to-ceiling windows, and state-of-the-art amenities. Perfect for those seeking sophisticated urban living at its finest.',
  ...
  squareFeet: 3500,
  images: [
    '/api/placeholder?w=800&h=600&text=Penthouse+Main',
    '/api/placeholder?w=800&h=600&text=Penthouse+Living',
    '/api/placeholder?w=800&h=600&text=Penthouse+Kitchen',
    '/api/placeholder?w=800&h=600&text=Penthouse+Bedroom',
  ],
  status: 'published',
  ownerId: 'owner1',
  owner: {
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          ...
          <div className="lg:col-span-1 h-fit space-y-4">
            ...
            {property.owner && (
              <div className="rounded-lg border border-border bg-card p-6">
                ...
                <div className="space-y-3">
                  <div>
                    <p className="font-semibold text-foreground">{property.owner.name || property.owner.email}</p>
            ...
            <div className="rounded-lg border border-border bg-card p-6 text-xs text-muted-foreground space-y-2">
              <p>
                <span className="font-medium text-foreground">Listed:</span> {formatDate(property.createdAt)}
              </p>
              <p>
                <span className="font-medium text-foreground">Status:</span> {property.status}
              </p>
              <p>
                <span className="font-medium text-foreground">Property ID:</span> {property.id}
              </p>
            </div>
          </div>
        </div>
